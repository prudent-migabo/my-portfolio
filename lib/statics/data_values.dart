class DataValues {
  // About App
  static const String appName = 'Prudent MIGABO';
  static const String appNameLong = 'prudentmigabo';

  // External links
  static Uri githubURL = Uri.parse('https://github.com/prudent-migabo');
  static Uri linkedinURL = Uri.parse('https://www.linkedin.com/in/prudent-migabo-a772bb214/');
  static Uri twitterURL = Uri.parse('https://twitter.com/PrudentMigabo?t=DVdPvOLIs2iWN25dwXCMjA&s=09');
  static Uri telegramURL = Uri.parse('https://t.me/prudentmigabo');
  static Uri facebookURL = Uri.parse('https://web.facebook.com/prudent.migabo.3');
  static Uri mesPiecesClientURL = Uri.parse('https://play.google.com/store/apps/details?id=com.techyahead.expoin');
  static Uri mesPiecesBusinessUrl = Uri.parse('https://play.google.com/store/apps/details?id=com.techyahead.expoinbusiness.mes_pieces_business');

  // Header
  static const String headerGreetings = 'Hello! I\'m';
  static const String headerName = 'Prudent MIGABO';
  static const String headerTitle = 'Software developer | Mobile application developer';

  // Navigation Bar
  static const String navBarAboutMe = 'About Me';
  static const String navBarProjects = 'Projects';
  static const String navBarSkills = 'Skills';
  static const String navBarContactMe = 'Contact Me';

  // About me
  static const String aboutMeTitle = 'About Me';
  static const String aboutMeDescription =
      'Hi, am a mobile application developer with experience in developing applications for both iOS and Android platforms. I have experience in developing applications using Flutter(Dart), Java, and Kotlin programming languages. I am passionate about creating user-friendly applications that provide a seamless user experience. I am also experienced in working with teams and collaborating with designers to create visually appealing applications.';
  static const String aboutMeBiographyTitle = 'Biography';
  static const String aboutMeBiographyDescription =
      'Hi there! I\'m a computing student, solution developer, and community volunteer. I believe that personal growth and learning are essential for success, and I have dedicated my life to achieving both. '
      'Because of that, I\'m always looking to learn new things and apply them to whatever I\'m doing. '
      'Technology is always igniting my passion for the positive impact it can have, and I\'m excited to continue growing my skills and knowledge, so I started my career as a techie. '
      'In addition to my studies, I\'m also passionate about giving back to various communities and helping others, which is why I volunteer my time to help out whenever I can, and I believe that this work is vital in helping to create a better future for everyone.';


  // Experience
  static const String experienceTitle = 'Projects';
  static const String experienceOrg1Title = 'MES Pieces Client';
  static const String experience1Description = 'Flutter | Firebase | Bloc | Shared Preferences';
  static const String experienceOrg1Description =
      "MES Pieces is a multi-sector company with a mission of financial facilitation without borders in time and space.\n "
      "With MES Pieces client you can make :\n"
      "- The purchase, sale, swap and payment of CRYPTOGRAPHIC Services.\n"
      "- The advertising, marketing and sale of products and tickets.";
  static const String experienceOrg2Title = 'MES Pieces Business';
  static const String experience2Description = 'Flutter | Firebase | Bloc | Shared Preferences';
  static const String experienceOrg2Description =
      "MES Pieces Business is the market place, it's in charge of providing to clients services including the purchase, sale, swap and payment of CRYPTOGRAPHIC Services";

  static const String experienceOrg3Title = 'ADA to Mobile';
  static const String experience3Description = 'Flutter | Firebase | Provider | RestAPIs | Hive | SharedPreferences';
  static const String experienceOrg3Description =
      "ADA to Mobile is a mobile application that allows the exchange of ADA cryptocurrencies received in a local currency from anywhere and the opposite, in Africa.";

  static const String experienceOrg4Title = 'XCooker';
  static const String experience4Description = 'Flutter | Bloc | RestAPIs | Hive | SharedPreferences';
  static const String experienceOrg4Description =
      "XCooker is an IoT(Internet of Things) mobile application based, it is an application that controls an induction cooker, with XCooker the user can: \n"
      "- Change the temperature of plaques.\n"
      "- Use a timer to automatically limit the cooking time.\n"
      "- Add a cooking mode.";

  //Skills
  static const String skillsTitle = 'Skills';
  static const String skillsTitle1 = 'Languages';
  static const String skillsTitle2 = 'Frameworks';
  static const String skillsTitle3 = 'Tools';
  static const String skillsTitle4 = 'Soft Skills';

  // Contact
  static const String contactTitle = 'Contact';
  static const String contactDescription =
      "I appreciate you looking at my public profile. Please feel free to ask me anything or to discuss anything with me. I'm happy to chat with and meet new people. So find me and get in touch with me. Don't hesitate to reach out - let's make something cool together.";
  static const String contactBanner = 'Send me an email at';
  static const String contactEmail = 'lyadungamigaboprudent@gmail.com';


}
